...
import { EmailData, FileAttachment, SuratJalan } from "@/lib/interface";
...
export default function FormCreatePage({ dataSurat }: FormCreateProps) {
  ...
  // Generate no surat
  useEffect(() => {
    if (!mode && !draftId && dataSurat && dataSurat.length >= 0) {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
        <div>
          ...
          <Input
            ...
            readOnly={!isEditMode} // Readonly untuk mode create, editable untuk edit
          />
        </div>
